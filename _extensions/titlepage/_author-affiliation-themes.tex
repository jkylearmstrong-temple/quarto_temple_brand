...
\newcommand{\titlepageauthorblock}{
$if(titlepage-theme.author-color)$\textcolor{$titlepage-theme.author-color$}$endif${\authorstyle{$if(by-author/allbutlast)$$for(by-author/allbutlast)$\nohyphens{$by-author.name.literal$}$for(by-author.affiliations)${\textsuperscript{$it.number$}}$sep$\textsuperscript{,}$endfor$$sep$$titlepage-theme.author-sep$ $if(titlepage-theme.author-space-between)$~\\ \vspace{$titlepage-theme.author-space-between$}$endif$$endfor$$if(by-author/allbutlast/rest)$,$endif$$for(by-author/last)$ and $if(titlepage-theme.author-space-between)$~\\ \vspace{$titlepage-theme.author-space-between$}$endif$\nohyphens{$by-author.name.literal$}$for(by-author.affiliations)${\textsuperscript{$it.number$}}$sep$\textsuperscript{,}$endfor$$endfor$$else$$for(by-author/last)$\nohyphens{$by-author.name.literal$}$for(by-author.affiliations)${\textsuperscript{$it.number$}}$sep$\textsuperscript{,}$endfor$$endfor$$endif$}}}
$endif$
...
$if(author)$
$if(false)$
% With titlepage-corresponding-only: true, list only authors marked
% corresponding: true; otherwise list every author with an email.
$endif$
$for(by-author)$$if(by-author.email)$$if(titlepage-corresponding-only)$$if(by-author.attributes.corresponding)$\titlepagecorrespondencetrue$endif$$else$\titlepagecorrespondencetrue$endif$$endif$$endfor$
...
$if(titlepage-corresponding-only)$
